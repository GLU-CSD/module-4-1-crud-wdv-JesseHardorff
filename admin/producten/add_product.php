...
<body>
    <form method="post">
        ...
        <button type="submit" name="submit" class="button4">Add</button>
    </form>
</body>
</html>
<?php
if (isset($_POST['submit'])) {
    ...
    $check_sql = "SELECT COUNT(*) FROM producten WHERE id = ?";
    ...
    } else if (!preg_match("/^[a-zA-Z0-9 .,\"-]+(?:'?[a-zA-Z0-9 .,\"-])*$/", $product_bio)) {
        echo "Ongeldige Bio. Gebruik alleen letters, cijfers, spaties, en de symbolen . , ' \" -";
    } else if (!preg_match("/^[a-zA-Z0-9 .,\"-]+(?:'?[a-zA-Z0-9 .,\"-])*$/", $product_kleinbio)) {
